...
export default function CreateEmployeePage() {
  ...
  const handleSubmit = async () => {
    ...
    setLoading(true);
    try {
      await aiEmployeesService.create({
        name: formData.name,
        role: formData.role,
        voiceModel: formData.voiceModel,
        language: 'English',
        instructions: formData.greeting || 'Be helpful and polite.'
      });
      navigate('/employees');
    } catch (error) {
...
<input className="w-full h-10 px-md bg-surface-container-lowest border border-outline-variant rounded-xl text-body-md focus:border-primary focus:ring-1 focus:ring-primary transition-all" id="name" placeholder="e.g. Sarah Jennings" type="text" value={formData.name} onChange={(e) => setFormData({...formData, name: e.target.value})}/>
</div>
{/* Primary Role */}
<div className="flex flex-col gap-xs">
<label className="font-label-md text-label-md text-on-surface" htmlFor="role">Primary Role</label>
<div className="relative">
<select className="w-full h-10 px-md bg-surface-container-lowest border border-outline-variant rounded-xl text-body-md appearance-none focus:border-primary focus:ring-1 focus:ring-primary transition-all" id="role" value={formData.role} onChange={(e) => setFormData({...formData, role: e.target.value})}>
...
<option value="Technical Concierge">Technical Concierge</option>
...
<textarea className="w-full px-md py-md bg-surface-container-lowest border border-outline-variant rounded-xl text-body-md focus:border-primary focus:ring-1 focus:ring-primary transition-all resize-none" id="greeting" placeholder="How should your AI answer the phone or start a chat?" rows={4} value={formData.greeting} onChange={(e) => setFormData({...formData, greeting: e.target.value})}></textarea>
<p className="font-label-sm text-label-sm text-outline">Character count: {formData.greeting.length}/500</p>
</div>
</div>
</section>
...
<div className={`group relative flex items-center p-md border-2 ${formData.voiceModel === 'Oliver' ? 'border-primary bg-primary-fixed/10' : 'border-outline-variant bg-surface-container-lowest'} rounded-xl cursor-pointer transition-all`} onClick={() => setFormData({...formData, voiceModel: 'Oliver'})}>
...
{/* Voice Card 2 */}
<div className={`group flex items-center p-md border-2 ${formData.voiceModel === 'Sophia' ? 'border-primary bg-primary-fixed/10' : 'border-outline-variant bg-surface-container-lowest'} rounded-xl cursor-pointer hover:border-primary hover:bg-surface-container-low transition-all`} onClick={() => setFormData({...formData, voiceModel: 'Sophia'})}>
<div className="flex-1">
...
{/* Voice Card 3 */}
<div className={`group flex items-center p-md border-2 ${formData.voiceModel === 'Kael' ? 'border-primary bg-primary-fixed/10' : 'border-outline-variant bg-surface-container-lowest'} rounded-xl cursor-pointer hover:border-primary hover:bg-surface-container-low transition-all`} onClick={() => setFormData({...formData, voiceModel: 'Kael'})}>
<div className="flex-1">
...
{/* Voice Card 4 */}
<div className={`group flex items-center p-md border-2 ${formData.voiceModel === 'Elena' ? 'border-primary bg-primary-fixed/10' : 'border-outline-variant bg-surface-container-lowest'} rounded-xl cursor-pointer hover:border-primary hover:bg-surface-container-low transition-all`} onClick={() => setFormData({...formData, voiceModel: 'Elena'})}>
<div className="flex-1">
...
<button className="px-xl py-md text-on-surface-variant font-label-md hover:text-primary transition-colors flex items-center gap-xs" onClick={() => navigate('/employees')}>
            Cancel
          </button>
<button className="bg-primary text-white px-xxl py-md rounded-xl font-label-md shadow-sm hover:shadow-md hover:bg-primary-container transition-all active:scale-95 disabled:opacity-50" onClick={handleSubmit} disabled={loading || !formData.name || !formData.role}>
...
<div>
<h4 className="font-label-md text-on-surface" id="preview-name">{formData.name || 'New AI Employee'}</h4>
<p className="font-label-sm text-outline" id="preview-role">{formData.role || 'Defining role...'}</p>
</div>
</div>
...
<p className="font-body-sm italic text-on-surface-variant" id="preview-greeting">"{formData.greeting || 'Your personalized greeting will appear here as you type...'}"</p>
</div>
{/* Visualization Asset */}
